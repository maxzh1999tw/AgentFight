...
namespace AgentFight
{
	class Agent
	{
		...
		public Agent()
		{
			...
			Attacktion = 10;
			AttackRange = 100;
			SetCooldown = new Dictionary<Action, int>() { { Action.Attack, 50 }, { Action.Defand, 20 } };
			Cooldown = new Dictionary<Action, int>() { { Action.Attack, 0 }, { Action.Defand, 0 } };
			Recover = new Dictionary<Action, int>() { { Action.Attack, 20 }, { Action.Defand, 50 }, { Action.Hurt, 20 }, { Action.Jump, int.MaxValue } };
		...
		public void Tick()
		{
			...
			if (ViewVersion < -70) ViewVersion = 70;
		}
	}
}
