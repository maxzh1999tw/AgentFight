...
namespace AgentFight
{
	class World
	{
		...
		{
			if (agent.Standing && agent.DoingDelate <= 0)
			{
				agent.Jump(power);
			}
		}

		public void Move(Agent agent, bool way)
		{
			if (agent.Doing == Agent.Action.Jump || agent.DoingDelate <= 0)
			{
				...
				if (way == Agent.RIGHT)
				{
					if (agent.Location.X + step + agent.Size.Width > Size.Width)
					{
						agent.Location = new Point(Size.Width - agent.Size.Width, agent.Location.Y);
					}
					else
					{
						agent.Location = new Point(agent.Location.X + step, agent.Location.Y);
					}
				}
				else
				{
					if (agent.Location.X - step < 0)
					{
		...
		{
			if (agent.Cooldown[Agent.Action.Attack] <= 0 && agent.DoingDelate <= 0)
			{
		...
		{
			if (agent.Cooldown[Agent.Action.Defand] <= 0 && agent.DoingDelate <= 0)
			{
